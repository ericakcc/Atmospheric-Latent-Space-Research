...
def main():
    ...
    config.experiment_name = "masked_vae_test"
    config.description = "使用地形掩碼的VAE測試版本，僅使用少量數據"
    ...
    config.model_config.update({
        "latent_dim": 64,  # 增加潛在空間維度以處理更高解析度
        "in_channels": input_channels,
        "input_height": input_height,
        "input_width": input_width,
        "conv1_out": 32,  # 增加特徵數以處理更高解析度
        "conv2_out": 64,
        "conv3_out": 128,
        "decoder_input_shape": [128, 64, 96],  # 調整解碼器輸入形狀
        # 確保解碼器輸出通道與輸入通道匹配
        "deconv3_out": input_channels
    })
    
    config.training_config.update({
        "batch_size": 8,  # 因為解析度提高，減少批次大小避免內存不足
        "learning_rate": 1e-4,
        "epochs": 500,
        "beta": 0.001,
        "loss_weights": {
